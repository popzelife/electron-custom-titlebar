...
const isElement = function (value) {
    ...
    return typeof value === 'object' && value !== null
        && value.nodeType === 1 && !isPlainObject(value);
};
...
class TitleBar extends EventEmitter {
    ...
    /**
     * setCurrentLabel into object to keep reference
     * @private
     * @param {String=} label - role attribute from tile
     */
    ...
    /**
     * getCurrentLabel from reference
     * @private
     * @returns {String}
     */
    getCurrentLabel() {
    ...
    displayActiveTile() {
        ...
        const _listenerMouseUp = function(e) {
            ...
            if (!container.contains(target)) {
                ...
                this._removeListennerMouseMouve();
        ...
        this._removeListennerMouseMouve = function() {
     ...
     */
    appendTo(context) {
        let contextFallback = context;
        if (!isElement(context)) {
            console.info('[Titlebar.js] Not a DOM Element provided - fallback to document.body');
            contextFallback = document.createElement('div');
            contextFallback.classList.add('titlebar-region');
            document.body.appendChild(contextFallback);
        }

        this.parentContext = contextFallback;
        this.computeTitlebar();
        contextFallback.appendChild(this.titlebarContainer);
    };
    ...
    update(options = {}) {
        this.destroy();
        this.init(options);
        this.appendTo(this.parentContext);
    }
     ...
     */
    destroy() {
        if (typeof this._removeListennerMouseMouve === 'function') {
            this._removeListennerMouseMouve();
        }
        if (typeof this._removeListennerMouseUp === 'function') {
            this._removeListennerMouseUp();
        }
